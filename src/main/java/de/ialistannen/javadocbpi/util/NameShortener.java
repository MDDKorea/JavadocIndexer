...
public class NameShortener {
   ...
   */
  public Map<String, String> shortenMatches(Set<String> names) {
    Map<String, String> typeFixpoints = findFixpoint(new HashSet<>(names));
  ...
  private Map<String, String> typeIteration(Map<String, String> names) {
    Map<String, List<Entry<String, String>>> types = names.entrySet().stream()
        .collect(
            toMap(
  ...
  private List<String> getAllParameterTypes(String input) {
    ...
    String parameterSection = input.substring(input.indexOf("(") + 1);
